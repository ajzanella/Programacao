...
baseretorno2<-cbind(nrasemp,nras,arqxml,nomearq)
...
while (i<=nrow(basemes)){
	...
	#base_sisco
write.table(base_sisco, file=paste(getwd(),"RP",paste("RPINC",baseretorno[i,23],".xml", sep=""), sep="/"), sep="\n", dec=",", row.names = FALSE, col.names = FALSE, quote = FALSE)

	i=i+1	
}


#write.table(imprimir, file=paste(getwd(),"RP",paste("RPINC",baseretorno[1,24],".txt", sep=""), sep="/"), sep="\n", dec=",", row.names = FALSE, col.names = FALSE, quote = FALSE)
