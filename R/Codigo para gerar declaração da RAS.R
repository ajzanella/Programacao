...
while (i<=nrow(basemes)){
	...
	base_sisco[2]<-paste("<IncluirRAS xmlns:xsi=","http://www.w3.org/2001/XMLSchema-instance"," xsi:noNamespaceSchemaLocation=","incluir_RAS.xsd",">",sep='"')
	...
	base_sisco[3+9 ]<-paste("<NumeroOperacaoEmpresa>",basemes[i,10],"</NumeroOperacaoEmpresa>", sep="")
...
#	base_sisco[3+18]<-paste("<VinculacaoNumDI>","</VinculacaoNumDI>", sep="")
	base_sisco[3+17]<-paste("</Operacao>", sep="")
	base_sisco[3+18]<-paste("<InfoComplementar>","</InfoComplementar>", sep="")
	base_sisco[3+19]<-paste("<CodigoMoeda>",basemes[i,9],"</CodigoMoeda>", sep="")
	base_sisco[3+20]<-paste("</IncluirRAS>", sep="")
